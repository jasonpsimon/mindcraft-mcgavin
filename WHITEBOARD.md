...
_Last updated: 2026-04-15 (D1 legacy-memory deprecation in progress; F+G queued for next session)_
...
### D1. Legacy `history.memory` deprecation (finish the `018e222` migration)

**Status:** 🔨 in progress 2026-04-15 • **Priority:** medium (active visible warning, small surgical change)

Upstream kolbytn/mindcraft ships a 500-char `history.memory` freeform summary generated by `promptMemSaving()` every 5 turns. The fork's commit `018e222 "Add episodic memory with semantic retrieval replacing lossy summaries"` intended to replace it, but only added episodic alongside — the legacy path still runs. Result: every 5 turns, bot makes an LLM call whose output is ONLY consumed by the `coding` prompt's `$MEMORY` placeholder, frequently truncates to 500 chars, and produces the visible log warning.

**Architectural justification.** ContextBuilder (`use_context_builder: true` already set in settings.js) routes conversation prompts to episodic + long-term memory directly at priority 6. The legacy summary is load-bearing only for the coding-prompt path via `replaceStrings`. Personality, rules, persistent rules, goals, seeded facts, and conversation recall all live in other systems (profile template, `self_prompter.persistentRules`, `long_term_memory`, `episodic_memory`) — `history.memory` is pure redundancy.

**Changes:**
1. `src/agent/history.js summarizeMemories()` — wrap the `promptMemSaving()` call in a `settings.use_context_builder` guard. When CB is on, skip the call; keep `episodic.addEpisode()` and `appendFullHistory()` as-is.
2. `ThatCoolGuyDude.json` — remove `$MEMORY` from the `coding` template. Coding prompts route through `messages` + `$CODE_DOCS` + `$EXAMPLES`.
3. `profiles/defaults/_default.json` — same edit for future profiles.
4. Leave `history.memory` field + `history.save()` persistence intact for backward compat with existing memory.json files.
5. Leave `$MEMORY` resolution in `replaceStrings()` intact — still used by other paths (NPC, goals, etc.).

**Success signal:** bot restarts, "Memory truncated to 500 chars" warning stops appearing in logs, no LLM call every 5 turns for memory saving, coding prompts still generate valid codeblocks.
