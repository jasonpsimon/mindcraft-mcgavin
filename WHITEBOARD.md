...
_Last updated: 2026-04-17 (BT-bundle(c) File I/O silent-swallow audit — moved to in-progress. Audit results: 21 I/O callsites (13 reads, 8 writes). All 13 reads log their catch branches. Four write-side silent-swallow / misleading patterns found: (1) **`src/models/prompter.js:134`** — **real latent bug**: sync `writeFileSync` called with a callback `(err) => throw ...` that the sync signature doesn't accept; callback is discarded; any profile-save failure skips the intended log and escapes as an uncaught throw. (2) **`src/agent/history.js:83`** — init `writeFileSync('[]')` sits *outside* the try/catch; a missing histories dir propagates into async `appendFullHistory()` called from `add()` with no catch → unhandled rejection, no `[History]` log. (3) **`src/agent/history.js:89`** — write inside a try/catch whose message reads `'Error reading ...full history file'`; a write-side failure gets labeled "reading." Misleading. (4) **`src/agent/vision/camera.js:62`** — screenshot `fs.writeFile` has no local try/catch and caller (`vision_interpreter.js`) also doesn't wrap; no `[Camera]` prefix at the write site. Two weak-log cases also tightened: `src/agent/npc/controller.js:45` (no prefix/error/filename) and `src/utils/keys.js:5` (parse errors look like missing-file). Plan: ~17-20 lines across 5 files. Out of scope: no JSONL sink, no new observability module, no retry logic, no readFileSync changes (all 13 already log). Blast radius: additive logging except `prompter.js:134` which is also a bug fix — no behavior changes on success paths. Final BT-bundle item; closes the file-I/O gap in the observability roadmap. Starts on top of BT-bundle(b) `46024a6`.)_
...
**Observability (7 modules + 8 inline-logging shipments live — BT-1, BT-2, BT-3, BT-4, BT-5, BT-6, BT-7, BT-8, BT-9, BT-10, BT-11, BT-12, Goal, BT-bundle(a), BT-bundle(b)):**
...
### BT-bundle(c). File I/O silent-swallow audit — tighten catch branches across 5 files

**Status:** 🚧 in progress (implementing 2026-04-17) • **Priority:** low (small, mechanical; third and final BT-bundle item) • **Source:** BT-bundle. Observability minor items; L3 audit (April 15) noted file-I/O was the last unaudited silent-failure surface

**Root cause.** The L3 audit and BT-bundle scope noted "27 `readFileSync` + 8 async `fs.readFile/writeFile` calls — audit each `catch` branch for 'logged or swallowed.'" Full re-enumeration 2026-04-17 found 21 I/O callsites on `develop` (13 reads + 8 writes; the L3 count included `existsSync` pre-checks which don't need error handling). All 13 read sites have catch-and-log. Four write sites have silent-swallow or misleading patterns, and two read sites log weakly (no prefix / no error / no filename, or parse errors look like missing-file). Under a disk-full, permission-denied, or malformed-JSON scenario, the affected paths produce no greppable structured line — the failure either crashes up into BT-bundle(b)'s `uncaughtException` handler (better than nothing, but the specific file and subsystem are lost) or logs a misleading message that points a postmortem reader in the wrong direction.

**Scope.** ~17-20 lines across 5 files:
- **`src/models/prompter.js`** (line 134): replace `writeFileSync(path, data, (err) => {...})` with a proper try/catch + `[Prompter]` structured log. **This is also a latent bug fix** — sync `writeFileSync` does not accept a callback; the `(err) => throw ...` arrow is silently discarded by Node. A real write failure (ENOSPC, EACCES) throws synchronously and the intended "Failed to save profile" error message never appears. ~5 lines.
- **`src/agent/history.js`** (line 83): wrap the `if (this.full_history_fp === undefined)` init `writeFileSync('[]')` in its own try/catch with `[History] init write failed` log. Keep it non-fatal — set `full_history_fp` back to `undefined` so a subsequent call will retry (or at least not wedge future appends). ~4 lines.
- **`src/agent/history.js`** (line 89): fix the misleading catch message at line 90 — `'Error reading ${this.name}'s full history file'` also covers write-side throws. Rename to `'Error appending to ${this.name}'s full history file'` (the catch wraps read→parse→push→write, "appending" accurately describes the whole operation). ~1 line change.
- **`src/agent/npc/controller.js`** (line 45): include filename + error message in the catch log. Current: `'Error reading construction file'`. New: `[NPC] Error reading construction file ${file}: ${e.message}`. ~2 lines.
- **`src/utils/keys.js`** (line 5): include error message so a parse error doesn't look like a missing file. Current: `'keys.json not found. Defaulting to environment variables.'` (always the same message regardless of error). New: branch on `err.code === 'ENOENT'` — if missing say the current message, otherwise say `'[Keys] keys.json read/parse failed: ${err.message}. Defaulting to environment variables.'`. ~3 lines.
- **`src/agent/vision/camera.js`** (line 62): wrap the `await fs.writeFile(${this.fp}/${filename}.jpg, buf)` with a try/catch, log `[Camera] screenshot save failed (${filepath}): ${err.message}`, and re-throw so the caller (`vision_interpreter.js` `capture()` → `lookAtPlayer` / `lookAtPosition`) still sees the failure. The local log point makes the filesystem failure visible at the camera layer instead of being unraveled by the unwinding stack. ~4 lines.

**Out of scope.** No JSONL sink (matches existing `[Boot] snapshot file write error`, `[ProtectedZone]`, `[MindServer]`, `[Prompter]`, `[Coder]` conventions — console-only for file-I/O failures). No new observability module. No retry / backoff logic (out of logging scope; a separate concern). No changes to the 13 read catch branches that already log — only the two weakly-logged reads (`controller.js:45` and `keys.js:5`) get a tighter message. No `existsSync` review (8 callsites, but `existsSync` doesn't throw — it returns boolean; failures are logical, not exceptional).

**Blast radius.** Five files, all additive logging except `prompter.js:134` which is also a bug fix that changes behavior only on the failure path (before: silent throw with no log; after: logged throw). No changes to success paths. Callsite enumeration:
- `prompter.js:134` — runs once per bot start inside the `Prompter` constructor. Success path unchanged (the `console.log("Copy profile saved.")` moves into the try-block success branch).
- `history.js:83` — fires on the first `appendFullHistory` call of each session (when `full_history_fp === undefined`). Success path unchanged.
- `history.js:89` — fires every time the history buffer flushes (`turns.length >= max_messages`). Success path unchanged (the append still succeeds; only the catch message is renamed).
- `controller.js:45` — fires once per `NPCController.init()` on agent start; only changes the catch message.
- `keys.js:5` — fires once per process at module import; only changes the catch message.
- `camera.js:62` — fires on every vision screenshot (`camera.capture()`) called from `vision_interpreter.js` `lookAtPlayer`/`lookAtPosition`. The re-throw preserves the caller's current failure-handling (there isn't any — it bubbles up into the vision command handler); the log just surfaces *where* in the vision pipeline the failure happened.

**Verification.** `node --check` parse-clean on all 5 files. Synthetic failure injection per site: `prompter.js` — call the constructor with a profile path whose directory is read-only (chmod); expect `[Prompter] Failed to save profile` log. `history.js:83` — point `full_history_fp`'s directory at a read-only path; expect `[History] init write failed` log. `history.js:89` — fill the disk partition (or use a `/proc`-based trick) while the full_history append fires; expect the renamed `'Error appending ...'` message. `controller.js:45` — rename `src/agent/npc/construction/` to force the readdirSync path to throw; expect `[NPC] Error reading construction file ...` with the filename. `keys.js:5` — write an invalid-JSON `keys.json`; expect `[Keys] keys.json read/parse failed: Unexpected token ...`. `camera.js:62` — point `this.fp` at a read-only directory; expect `[Camera] screenshot save failed (...)` with the path and error. Live: none required — these fires on failure only; no natural trigger needed for observability-level confirmation. Bot-start smoke test confirms the happy path is unchanged.

**Downstream unblock.** Closes the file-I/O gap in the observability roadmap — every user-visible file-I/O operation now emits a structured `[<Subsystem>] ...` line on failure, matching the `[StateTicker]` / `[BootSnapshot]` / `[LLM]` / `[Exit]` conventions. With BT-bundle(a) (mutex wait), BT-bundle(b) (process exit reasons), and BT-bundle(c) shipped, the BT-bundle is complete. Post-BT-bundle roadmap: BT-3b (19 LLM adapter sweep) and BT-7b (~72 remaining skill wraps) both remain trigger-gated by "more adapters/skills actually firing in live play"; nothing new on the log-roadmap side.
...
- ~~**File I/O silent-swallow scan.** 27 `readFileSync` + 8 async `fs.readFile/writeFile` calls. Audit each `catch` branch for "logged or swallowed." Already noted in L3 audit (April 15). Risk: silent memory-save failures. Extends #15 (`full_state.js` sweep, shipped) to the whole codebase.~~ (in progress as BT-bundle(c) — see In-progress: BT-bundle(c))
