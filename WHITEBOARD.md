ssh: Could not resolve hostname claude-sync: Name or service not known
scp: Connection closed
# mindcraft-mcgavin — Project Whiteboard

Digital workspace for mindcraft-mcgavin bot development. Holds current state, active work, to-do queue, recent history, and known-but-deferred issues. Update freely as work lands — this is meant to be edited, not preserved.

_Last updated: 2026-04-17 (BT-bundle(c) File I/O silent-swallow audit — **shipped `4b7fc33`**. Six fix sites across 5 files, ~17 lines of real change + explanatory comments. One real latent bug fix: `src/models/prompter.js:134` — sync `writeFileSync` was called with a callback `(err) => throw ...` that the sync signature doesn't accept; the arrow was silently discarded by Node, so any profile-save failure escaped as an uncaught throw with no `[Prompter]` log. Wrapped in try/catch + re-throw. Three observability tightenings on write paths: `src/agent/history.js:83` init write wrapped with `[History] init write failed` log + `full_history_fp` reset for retry; `src/agent/history.js:89` catch message renamed from "Error reading..." → "Error appending to..." (the try-block covers read→parse→push→write, the misleading "reading" label pointed postmortem readers away from write-side failures); `src/agent/vision/camera.js:62` screenshot write wrapped with `[Camera] screenshot save failed` + re-throw. Two weak-log reads tightened: `src/agent/npc/controller.js:45` got a `[NPC]` prefix + error message (prior log was prefix-less / message-less / path-less); `src/utils/keys.js:5` now branches on `err.code === 'ENOENT'` so parse errors no longer look like missing-file. Verification: 14/14 synthetic checks in `test_btc.mjs` (read-only chmod, nonexistent paths, synthetic `SyntaxError`) confirmed every new prefix/message/code fires; `node --check` parse-clean on all 5 files; bot-restart smoke test on `4b7fc33` observed `ThatCoolGuyDude spawned` + StateTicker normal + zero BT-bundle(c) error lines (happy path unchanged). The BT-bundle is now complete. Post-BT-bundle roadmap: BT-3b (19 LLM adapter sweep) and BT-7b (~72 remaining skill wraps) remain trigger-gated by live-play signal.)_
...
_(empty — BT-bundle(c) File I/O silent-swallow shipped `4b7fc33` and live-deployed 2026-04-17 on top of BT-bundle(b). See Recently completed. **The BT-bundle is now complete.** Post-BT-bundle roadmap: BT-3b (19 LLM adapter sweep) and BT-7b (~72 remaining skill wraps) both remain trigger-gated by live-play signal — neither is promoted to in-progress until the corresponding adapter/skill actually fires enough in live play to matter.)_
