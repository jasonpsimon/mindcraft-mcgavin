...
_(none — BT-30 between sub-tickets; BT-30c queued.)_

## Shipped — awaiting live verification

### 30. Bot mode profiles: survivor / assistant-server / assistant-user / auto

**Status:** 🚢 shipped 2026-04-22 awaiting live verification (`ed49817`, BT-30b — Assistant variants + sticky-return; layered on BT-30a `a903746`) — real `onPlayerOnlineChange` replaces the BT-30a stub: `assistant-server` pauses self_prompter on first player join (other-count 0→1) and resumes on last leave (1→0); `assistant-user` pauses/resumes only on the named user (read from profile JSON `assistant_user`, settable atomically via `!botMode assistant-user <username>`); `auto` sticky-return flips runtime survivor→assistant on any join (leave is intentionally NO-OP — BT-30c's 5-min idle timer owns the assistant→survivor drop-back). New `reconcileOnSpawn()` fires from agent.js spawn handler post-settle to handle the boot-time-with-existing-players case (mineflayer doesn't fire `playerJoined` for pre-existing players). New `assistant_user` field threaded through ModeProfile + getJson/loadJson + dual-writeback to profile JSON. `!botMode` accepts optional username param; missing-username on `assistant-user` returns operator chat error with no state change. self_prompter NOT modified — code review found `pause()` puts state in PAUSED, watchdog only fires on STOPPED+circuitBreaker, so the planned `stoppedReason='assistant_player_online'` taxonomy entry was unnecessary by construction. `[ModeProfile]` log lines on every transition + pause/resume + reconcile outcome. BT-30c (5-min inactivity timer) and BT-30d (Survivor goal queue) remain queued. • **Priority:** medium (user-facing control + structured autonomy)
