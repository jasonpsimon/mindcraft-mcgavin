...
**Status:** 🛠️ in progress 2026-04-20 • **Priority:** low (quality-of-life + anti-cheat resilience)

**Picked up from to-do queue.** Bot currently issues commands back-to-back as fast as the LLM + mineflayer permit. Adds per-command post-execution delay inside `executeCommand` at `src/agent/commands/index.js:221` — the single chokepoint every LLM-issued command passes through. Mode-triggered reflexes (self_preservation, self_defense) call bot/skill methods directly, never through `executeCommand`, so they stay instant.

**Plan:**
- New module `src/agent/human_delays.js` exporting `getHumanDelay(commandName)` → ms. Command-name → category map; category → delay range; uniform random draw for jitter.
- Four categories: trivial (1–2s, read-only queries), movement (2–3s), gather/craft (3–5s), complex (4–6s). Plus an instant allowlist (`!stop`, `!stfu`, `!restart`, `!clearChat`, emergency/meta) that returns 0.
- Unknown-command fallback: movement tier (conservative middle). Keeps the module degrade-gracefully if the actions.js registry drifts.
- Wire: one line inside `executeCommand` after `command.perform` returns — `await sleep(getHumanDelay(parsed.commandName))`.
- Logs: `[HumanDelay] !X waiting Nms` when delay > 0, observability-first.

**Acceptance.** `node --check` clean; bot reboots clean; natural trigger (LLM issues any command in conversation) shows `[HumanDelay]` line in tmux capture and measurable pause between commands.

_(All other previous items stable: L1.4-wire BT 2 shipped `ea48e1d`; #12 Movements safety audit closed `b4f0190`; #27 D1 migration closed `5663aa2`; #12-follow-up shipped `85d93f7`; #2 Layer 3 shipped `71df242`; #2 Layer 2 + #3 Swamp biome closed as redundant. Twenty-two items awaiting live verification on natural triggers. 2026-04-16 optimization-audit bundle closed at 6/6.)_
