...
**Status:** 🚢 shipped — full BT-30 series complete, awaiting live verification. BT-30a (`a903746`) + BT-30b (`ed49817`) + BT-30c (`d8299ce`) + BT-30d (`c61f680`) + BT-30e (`185e97b`) all landed 2026-04-22 → 2026-04-23. See per-stage as-built recap in the BT sequence below; the most recent ship-note recap lives in `Current state (live on develop)` HEAD. • **Priority:** medium (user-facing control + structured autonomy)
...
- **BT-30e.** ✅ shipped `185e97b` 2026-04-23. Docs close-out — three pieces. (1) `actions.js` `!botMode` description rewritten as full LLM-facing explanation: names all four configured profiles, calls out runtime-vs-configured for `auto`, explains survivor goal-queue interaction with `!goal` (manual goals splice at queue head), documents assistant-user `<username>` arg, notes auto 30-min idle flip. (2) `profiles/defaults/_default.json` — added `"mode_profile": "auto"` and `"assistant_user": ""` between `speak_model` and `modes` blocks as canonical schema entries. (3) Live `ThatCoolGuyDude.json` — added explicit `"mode_profile": "auto"` and `"assistant_user": ""` so the bot stops emitting the absent-field warn on every boot.
