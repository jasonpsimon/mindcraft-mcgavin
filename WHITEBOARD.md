...
_Last updated: 2026-04-17 (BT-3b LLM adapter sweep — **in progress**. JP approved promoting BT-3b off the trigger-gated list to finish the Principle 5 migration started by BT-3. Scope: wrap the 19 remaining model adapters (`azure, cerebras, claude, deepseek, gemini, glhf, gpt, grok, groq, huggingface, hyperbolic, mercury, mistral, novita, ollama, openrouter, qwen, replicate, vllm`) with `withLLMMetrics` so every adapter emits one uniform `[LLM]` line per round-trip matching `lmstudio.js` (BT-3). Research phase complete — all 19 adapter files read and shape-categorized: 12 OpenAI-compatible (default `extractUsage` works) + 7 requiring custom `extractUsage` or null-fields (claude, gemini, mistral, replicate, ollama, hyperbolic, huggingface). Special cases identified: `gpt.js` dual-path (both `chat.completions.create` and `responses.create` wrapped); `azure.js` inherits from GPT automatically; internal retry-loop adapters (glhf, qwen-embed, ollama, hyperbolic, huggingface) — wrap goes inside the loop so each attempt emits one line. No code written yet. Verification plan: `node --check` all 19 + synthetic log-capture for a Shape A and Shape B sample + LM Studio regression check. Live verification is partial by construction — JP's only active provider is LM Studio; the other 18 are synthetic-proven only, per the BT-6/BT-9/BT-10/BT-bundle acceptance pattern.)_
...
- Branch: `develop` — HEAD `4b7fc33`. Sixteen observability items shipped on `develop` today: BT-1 StateTicker, BT-8 BootSnapshot, BT-3 LLM call telemetry (+ BT-3b **in progress** — 19 remaining adapters being migrated this session), BT-2 DamageStream, BT-12 startup-window ordering fix, BT-4 MemoryRecall, BT-5 AutoRecovery stats, BT-7 Skill lifecycle (+ BT-7b filed for remaining 72 skills), Goal lifecycle, BT-6 Pathfinder telemetry, BT-11 ContextBuilder truncation decisions, BT-9 World/time events, BT-10 Entity delta stream, BT-bundle(a) Mutex wait duration, BT-bundle(b) Process exit reasons, and BT-bundle(c) File I/O silent-swallow. Pushed to `origin/develop` 2026-04-17. **The lifecycle layer (BT-7 skills + Goal + BT-6 paths) sits underneath the measurement layer (BT-5) as the two-tier observability story, with BT-11 closing the prompt-construction counterpart alongside BT-4. BT-9 + BT-10 together surface ambient world/entity state (weather / time / dimension / nearby hostile lifecycle) that was previously invisible to log tailers. BT-bundle(a) adds a latency number to the existing `[BotMutex]` queue-depth log so concurrency-bug signal (growing queue) now pairs with a scale signal (how long acquires actually waited). BT-bundle(b) closes the end-of-session observability gap — every shutdown path that Node naturally reaches (`cleanKill`, uncaught throw, unhandled rejection, natural event-loop drain) now emits a structured `[Exit]` line with reason paired to exit code, so session-replay readers see *why* and *how* the tmux buffer ended instead of a dropped cursor. BT-bundle(c) closes the last unaudited silent-failure surface — six file-I/O catch branches tightened (one of them a real latent bug in `prompter.js:134` where a callback was being silently discarded on a sync `writeFileSync`). Every user-visible file-I/O operation now emits a structured `[<Subsystem>] ...` line on failure, matching the `[StateTicker]` / `[BootSnapshot]` / `[LLM]` / `[Exit]` conventions. The BT-bundle is now complete.**
...
**Observability (7 modules + 9 inline-logging shipments live — BT-1, BT-2, BT-3, BT-4, BT-5, BT-6, BT-7, BT-8, BT-9, BT-10, BT-11, BT-12, Goal, BT-bundle(a), BT-bundle(b), BT-bundle(c)):**
...
- **Mutex wait duration (BT-bundle(a)):** `src/agent/bot_mutex.js` `withLock` captures `enterT = Date.now()` + `let queued = false` right before the FIFO wait loop; flips `queued = true` inside the loop body so it trips only for acquires that actually waited; appends `${queued ? ` wait=${waitMs}ms` : ''}` to the existing `#<id> acquired: <label>${qd > 0 ? ` (queue: ${qd})` : ''}` log line. Uncontended acquires stay byte-identical (no `Date.now()` called, no extra template output); reentrant early-return path unchanged; release accounting unchanged. No new state on the mutex object, no JSONL sink (console-only matches the existing `[BotMutex]` convention).
- **Process exit reasons (BT-bundle(b)):** four `[Exit]` log points covering every shutdown path the Node runtime naturally reaches. `src/process/init_agent.js` (child-process entry point) installs three module-top-level handlers above the IIFE: `process.on('exit', code => console.log('[Exit] event=process_exit code=<n>'))` — synchronous, fires once per process for clean exits, event-loop drain, and post-`process.exit()` teardown; `process.on('uncaughtException', err => ...)` logs `event=uncaught err="<msg>" stack_head="<first-frame>"` then `process.exit(1)`; `process.on('unhandledRejection', reason => ...)` logs `event=unhandled_rejection reason="<msg>"` then `process.exit(1)`. `src/agent/agent.js` `cleanKill(msg, code)` prepends `console.log('[Exit] event=clean_kill code=<n> reason="<msg>"')` before the existing history/chat/save/`process.exit` cascade. Result: clean shutdowns emit two structured lines (`clean_kill` + `process_exit`); crashes emit two (`uncaught` or `unhandled_rejection` + `process_exit`). The explicit `process.exit(1)` in the crash handlers is load-bearing — registering a listener on `uncaughtException`/`unhandledRejection` suppresses Node's default crash-exit code, and `agent_process.js:31` parent-side restart logic (`if (code !== 0 && signal !== 'SIGINT') { restart }`) depends on the non-zero exit to decide whether to relaunch the child. Without the explicit exit, a crash would silently exit 0 and the supervisor would treat it as a clean shutdown. Signal-delivered shutdowns (SIGTERM / SIGINT / `tmux kill-session`) bypass `process.on('exit')` entirely in Node when no signal handler is installed — documented and accepted under the "no shutdown-semantics changes" scope boundary.
- **File I/O silent-swallow (BT-bundle(c)):** six file-I/O catch branches tightened across 5 files — ~17 lines of real change + explanatory comments. One real latent bug fix: `src/models/prompter.js:134` — sync `writeFileSync` was being called with a callback `(err) => throw ...` that the sync signature doesn't accept; the arrow was silently discarded by Node, so any profile-save failure escaped as an uncaught throw with no `[Prompter]` log. Wrapped in try/catch, logs `[Prompter] Failed to save profile <path>: <err.message>`, re-throws so the caller still sees the failure. Three observability tightenings on write paths: `src/agent/history.js:83` — init `writeFileSync('[]')` was outside the existing try/catch; wrapped with its own try/catch, logs `[History] init write failed <path>: <err.message>`, resets `full_history_fp = undefined` so a later call may retry, returns gracefully. `src/agent/history.js:89` — catch message read `'Error reading ...full history file'` but the try-block covered read→parse→push→write; a write-side throw (ENOSPC, EACCES) was mislabeled "reading." Renamed to `'Error appending to ...full history file'` so the label matches the try-block. `src/agent/vision/camera.js:62` — screenshot `fs.writeFile` had no local try/catch and the caller (`vision_interpreter.js`) doesn't wrap either; wrapped with try/catch, logs `[Camera] screenshot save failed (<path>): <err.message>`, re-throws. Two weak-log reads tightened: `src/agent/npc/controller.js:45` — prior log was `'Error reading construction file'` with no prefix / no error message / no path; now `[NPC] Error reading construction files from src/agent/npc/construction: <err.message>`. `src/utils/keys.js:5` — prior log was `'keys.json not found. Defaulting to environment variables.'` for every error class (a parse error looked identical to a missing file); now branches on `err.code === 'ENOENT'` — ENOENT keeps the original message, parse/permission errors emit `[Keys] keys.json read/parse failed: <err.message>. Defaulting to environment variables.`. New `[Prompter]` / `[History]` init / `[NPC]` / `[Keys]` / `[Camera]` structured prefixes make every file-I/O failure greppable from the tmux buffer. No new module, no JSONL sink, no retry/backoff logic, no `readFileSync` changes beyond the two weak-log reads. Ships on top of BT-bundle(b) and completes the BT-bundle.
- All observability emitters audited for Rule 7: grep for `bot.(dig|placeBlock|chat|toss|setControlState|attack|equip|unequip|activateItem)|pathfinder.goto` returns zero matches across `src/observability/state_ticker.js`, `src/observability/boot_snapshot.js`, `src/observability/damage_stream.js`, `src/observability/recall_log.js`, `src/observability/skill_lifecycle.js`, `src/observability/path_telemetry.js`, `src/agent/auto_recovery.js` (BT-5 stats), `src/utils/retry.js` (BT-3 `withLLMMetrics`), `src/memory/context_builder.js` (BT-11 inline decision logging), `src/agent/event_pipeline.js` (BT-9 World events + BT-10 Entity events — all new console.log calls sit alongside existing reads, zero bot mutation added; the `_trackedEntities` Map is in-process metadata only), `src/agent/bot_mutex.js` (BT-bundle(a) — the only new work inside `withLock` is a `Date.now()` read, a local flag, a subtraction, and a template literal append; the mutex's own state machine (`_holderToken`, `_holderLabel`, `_queue`, `_acquireCount`) and reentrant early-return path are untouched), `src/process/init_agent.js` (BT-bundle(b) — three `process.on(...)` handlers at module top level; each handler does only string interpolation + `console.log` and, for the two crash handlers, a terminal `process.exit(1)`; no mineflayer API touched, no `bot` reference exists at this scope, the existing `(async () => {...})()` IIFE is unchanged), and `src/agent/agent.js` `cleanKill` (BT-bundle(b) — one prepended `console.log` with a quoted template literal before the existing `history.add` / `bot.chat` / `history.save` / `state_ticker.stop` / `process.exit` sequence, which itself is untouched), `src/models/prompter.js` (BT-bundle(c) — wrapped an already-broken `writeFileSync` + callback pattern in a proper try/catch + re-throw; no bot reference at this scope, zero mineflayer API touched, existing `console.log("Copy profile saved.")` preserved on the success branch), `src/agent/history.js` `appendFullHistory` (BT-bundle(c) — two additive changes: a new try/catch around the init `writeFileSync('[]')` that either logs + resets `full_history_fp = undefined` for retry or returns gracefully, and a one-word rename of the existing catch message; neither touches the bot, the read/write itself, or any downstream append logic), `src/agent/npc/controller.js` `init()` (BT-bundle(c) — one-line tightening of the existing catch from `'Error reading construction file'` → `\`[NPC] Error reading construction files from src/agent/npc/construction: ${e.message}\``; the `readdirSync` / `readFileSync` / `JSON.parse` try-block body is unchanged, as is the downstream block-padding loop and the `bot.on('idle', ...)` handler registration), `src/utils/keys.js` (BT-bundle(c) — module-top try/catch at import time; prior single-message catch branched into `err.code === 'ENOENT'` vs. everything else with a distinct `[Keys]` prefix for parse errors; `getKey` / `hasKey` exports unchanged, `readFileSync`/`JSON.parse` pipeline unchanged, no bot reference exists at this scope), and `src/agent/vision/camera.js` `capture()` (BT-bundle(c) — one try/catch around the existing `await fs.writeFile(filepath, buf)` call; the `imageStream = this.canvas.createJPEGStream(...)` / `getBufferFromStream` / `_ensureScreenshotDirectory` / `console.log('saved', ...)` sequence is unchanged; the `throw err` inside the new catch preserves the caller's existing failure-unwinding behavior — `vision_interpreter.js` still sees the failure bubble up, just with a `[Camera]` log stamp at the write site first). BT-12 is an ordering fix in `agent.js`, no new module. Goal lifecycle is inline logging in `src/agent/self_prompter.js` + `src/agent/commands/actions.js` — no new module, no bot mutation. BT-9 time_phase logging is inline in `agent.js` `startEvents()`, alongside existing synthetic event re-emits; the four branches add console.log only. BT-bundle(c) is strictly file-I/O catch-branch logging — five files, six fix sites, zero new state, zero new listeners, zero mineflayer API touched; `prompter.js:134` is the one spot with a behavior change on the failure path (was silently throwing with no log, now logs + re-throws — the success path is byte-identical).
...
**Status:** 🟡 in progress (research complete, implementation pending) • **Priority:** medium (Principle 5 migration finish — dormant code paths)

**Why now.** JP promoted BT-3b off the trigger-gated list to finish the migration BT-3 started. The `withLLMMetrics` wrapper has been live on LM Studio since BT-3 shipped, so the wrapper's `[LLM]` line format is proven. Migrating the other 19 adapters with the same wrapper closes the observability gap uniformly and satisfies Principle 5 (finish the migration, kill redundant error-handling variants).

**Research phase — complete.** All 19 adapter files read (`src/models/*.js`) and shape-categorized:

- **Shape A (12 OpenAI-compatible, default `extractUsage` works):** `azure.js`, `cerebras.js`, `deepseek.js`, `glhf.js`, `gpt.js`, `grok.js`, `groq.js`, `mercury.js`, `novita.js`, `openrouter.js`, `qwen.js`, `vllm.js`. Each wrap: `withLLMMetrics({ label, model }, () => <original call>)` around `chat.completions.create` and `embeddings.create` where present.
- **Shape B (7 require custom `extractUsage` or accept null-fields):** `claude.js` (Anthropic `resp.usage.{input_tokens, output_tokens}` + `resp.stop_reason`), `gemini.js` (Google `result.usageMetadata.{promptTokenCount, candidatesTokenCount, totalTokenCount}`), `mistral.js` (OpenAI-compat shape but private `#client` field), `replicate.js` (streaming async iterator, no usage → null fields), `ollama.js` (raw fetch, `{eval_count, prompt_eval_count}`), `hyperbolic.js` (raw fetch to OpenAI-style endpoint), `huggingface.js` (streaming, no usage → null fields).

**Special cases called out:**
- `gpt.js` has **two call sites** — `chat.completions.create` (custom URL path) and `responses.create` (no-URL path). Both wrap.
- `azure.js` extends GPT and inherits the wrap automatically — no direct edit.
- Internal retry-loop adapters (`glhf.js`, `qwen.js` embed, `ollama.js`, `hyperbolic.js`, `huggingface.js`) have `maxAttempts=5` loops inside `sendRequest`. Wrap goes **inside** the loop body so each attempt emits one `[LLM]` line (terminal-error or success).
- Adapters with no embed throw as-is: claude, deepseek, cerebras, glhf, grok, groq, huggingface, hyperbolic, novita, openrouter, vllm, azure.

**Implementation scope.** ~25–40 additive lines across 19 files. One `withLLMMetrics(...)` wrap per async API call. No behavior changes — success path returns raw response unchanged; retry logic unchanged; no new sink; no new module.

**Verification plan.**
- `node --check` parse-clean on all 19 adapter files.
- Synthetic log-capture test: mock a Shape A response (e.g., `mercury.js`) and a Shape B response (e.g., `claude.js` + `gemini.js`) to confirm `[LLM]` line format matches `lmstudio.js`.
- Live regression: bot restart on gaming server, confirm LM Studio's `[LLM]` lines still fire (regression check, since `lmstudio.js` is already shipped).
- Live verification for the other 18 is **partial by construction** — JP's only active provider is LM Studio. The 18 non-LMStudio wraps ship synthetic-proven only, matching the BT-6/BT-9/BT-10/BT-bundle acceptance pattern where trigger-gated code paths accept synthetic verification.

**Out of scope.** No retry logic changes, no behavior changes, no new sink beyond console `[LLM]` lines, no new module.

**Blast radius.** Adapter classes are instantiated by `prompter.js` based on profile config. Each instance's `sendRequest` / `embed` is the wrap target. Callers (prompter, embeddings cache) see the raw response unchanged — wrap is transparent. No external consumers outside `src/models/*` import these adapters directly.

**Rule 7 audit commitment.** Each adapter edit adds only: one import line + one `withLLMMetrics(...)` wrap per API call site + (for Shape B) one local `extractUsage` callback. Zero mineflayer API touched. Zero new state. Zero new listeners. Success path byte-identical to pre-wrap behavior.
...
**Status:** 🟡 in progress (research complete, implementation pending — see **In-progress** section above for active scope) • **Priority:** medium (Principle 5 migration finish — promoted off trigger-gated 2026-04-17)
...
### BT-7b. Skill lifecycle — wrap remaining ~72 exports in `skills.js`

**Status:** ⏳ not started • **Priority:** low-medium (Principle 5 finish-migrations follow-up to BT-7)

**Scope.** BT-7 shipped `40c04f3` and wrapped the 11 hottest exports in `src/agent/library/skills.js`: `collectBlock`, `craftRecipe`, `smeltItem`, `pickupNearbyItems`, `placeBlock`, `equip`, `safeToss`, `safeTossBatch`, `goToNearestBlock`, `digDown`, `digUp`. The file has ~82 exported async skills total — the remaining ~71 still use ad-hoc `bot.chat` / bespoke `[Skills]` logs (or nothing) and do NOT produce uniform `[Skill]` lifecycle lines. This entry tracks the remaining sweep so Principle 5 is explicitly satisfied.

**Why deferred.** Live-run data from BT-7 is worth collecting first. The wrapper's outcome taxonomy (`success | error | abort`), 80-char args cap, and `argSelector` default (drop first positional) are all design decisions that should bake for a session before we lock them into 70+ more wraps. Any adjustment is cheaper to make on 11 sites than on 80.

**Trigger.** Ship BT-7b when any of the following is true:
- BT-7's live-stream data reveals a taxonomy or args-cap tweak worth making first.
- One session's worth of `data/skill-stream.jsonl` has been reviewed and the wrapper format is confirmed sound.
- A reader decides the mechanical sweep is worth doing regardless.

**Implementation sketch.**
1. Inventory remaining exports: `grep -nE '^export async function ' src/agent/library/skills.js` minus the 11 already wrapped.
2. For each, apply the rename-impl pattern: `export async function X(...)` → `async function _impl_X(...)` + `export const X = wrapSkill('X', _impl_X, {...opts});`
3. Where the first positional is not `bot`, set `argSelector: (args) => args`.
4. Strip per-function ad-hoc `[Skills]` log lines that are now redundant with the uniform line (Principle 5 — kill redundancy).
5. `node --check src/agent/library/skills.js` after each batch of ~10.
6. Ship, live-verify one sample from each broad category (movement / mining / inventory / combat / info / meta).

**Blast radius.** Per-function localized. Wrapper is proven. Same rename-impl pattern as BT-7; no new infrastructure.
...
- ~~**File I/O silent-swallow scan.** 27 `readFileSync` + 8 async `fs.readFile/writeFile` calls. Audit each `catch` branch for "logged or swallowed." Already noted in L3 audit (April 15). Risk: silent memory-save failures. Extends #15 (`full_state.js` sweep, shipped) to the whole codebase.~~ (shipped `4b7fc33` as BT-bundle(c) — see Recently completed)
- ~~**Process exit reasons.** `agent.js cleanKill` + `process.on('exit', ...)` — log the exit reason as a structured line so session replay sees the end clearly.~~ (shipped `46024a6` as BT-bundle(b) — see Recently completed)
...
**Next in the logging roadmap:** BT-bundle complete. BT-3b (19 LLM adapter sweep) and BT-7b (~72 remaining skill wraps) both remain trigger-gated by "more adapters/skills actually firing in live play"; neither is promoted until the corresponding subsystem actually fires enough in live play to matter. Nothing new on the observability side of the roadmap.
