...
- Branch: `develop` — HEAD `c61f680` (BT-30d Survivor tier-up goal queue; WB shipped-move lands on top of this). Most recent code ship 2026-04-23: **#30 Bot mode profiles — BT-30d Survivor tier-up goal queue** (`c61f680`) — parallel `ModeProfile.survivorQueue` of `{goal, predicate, source}` entries plus `headIndex`. Seed ladder is per-tier (full set before advancing): wood pickaxe/sword/axe → stone pickaxe/sword/axe → iron pickaxe/sword/axe + helmet/chestplate/leggings/boots + shield → diamond pickaxe/sword/axe + helmet/chestplate/leggings/boots → netherite pickaxe/sword/axe + helmet/chestplate/leggings/boots (28 entries, exported as `SEED_LADDER`). Predicates plain item-name `{has: '<name>', min: N}` evaluated against `bot.inventory.slots` (covers armor slots 5-8 + offhand 45 — `items()` alone would miss equipped armor). Persistence at `bots/<agent.name>/survivor_queue.json` — load lazily on first arm, seed-from-SEED_LADDER if absent/malformed, writeFileSync after every advance/insert. Tick cadence 30s (`SURVIVOR_TICK_MS`, `.unref()`-ed) armed only when `runtime === 'survivor'`; immediate first tick after arming. Wired arm/disarm into every transition: setConfigured, `_dropToSurvivor` (BT-30c idle drop), auto sticky-return + chat-return (disarm), reconcileOnSpawn (arm survivor + auto-no-players, disarm auto-with-players). Each tick: defensive disarm on drift; natural-completion advance via self_prompter STOPPED + stoppedReason='user' + `_lastPushedGoal` matching head; predicate-satisfied advance; otherwise push head if self_prompter is STOPPED. Manual `!goal "<text>"` while runtime=survivor calls new `manualSurvivorInsert` which splices `{goal, predicate: null, source: 'manual'}` at `headIndex`, persists, force-pushes — operator goal jumps the line; previous head shifts down by one. Null-predicate manual entries advance only on natural completion. `actions.js` `!goal` handler branches on `agent.mode_profile?.runtime === 'survivor'`; assistant runtimes keep the existing self_prompter.start path unchanged. New `[ModeProfile]` log lines: `survivorQueue load fp=…` / `survivorQueue seed`, `survivor-timer armed tick_ms=30000 head=… queue_len=…` / `survivor-timer disarmed`, `survivorQueue push reason=…`, `survivorQueue advance reason=natural_completion|predicate_satisfied`, `survivorQueue manual-insert head=… queue_len=…`. Awaiting live verification: cold boot in survivor profile fires `survivor-timer armed` + `survivorQueue seed` + push of "Craft a wooden pickaxe"; ladder advances on each crafted item; manual `!goal` splices at head and pushes immediately. Prior ship 2026-04-22: **#30 Bot mode profiles — BT-30c auto idle-timeout drop-back** (`d8299ce`) — 30-minute sliding window of chat-to-bot activity. New `noteActivity(source)` on `ModeProfile` records every qualifying event and resets `lastActivityAt`; `_startIdleTimer/_stopIdleTimer/_checkIdle` handle a 60s-cadence interval armed only when `configured=auto && runtime=assistant`; `_dropToSurvivor` flips runtime assistant→survivor and resumes self_prompter when no qualifying chat has landed in `IDLE_MS` (30 min). Drop fires regardless of player presence — that's the whole point of auto. `noteActivity` also handles auto chat-return (re-grab): a chat-to-bot landing while auto is in survivor flips runtime back to assistant + pauses self_prompter, inverse of the idle drop. `agent.js` chat handler split: whispers always count as activity (DM by definition); open chat counts only when the message contains the bot's name (case-insensitive substring) — ambient in-range chatter does NOT reset the window. `reconcileOnSpawn` auto branch arms the timer when boot lands in auto+assistant. `[ModeProfile] runtime assistant->survivor reason=auto_idle_timeout idle_ms=...` log on every drop; `auto_chat_return` log on every re-grab. Awaiting live verification: `!botMode auto` → 30-min idle without addressing the bot logs `auto_idle_timeout` and resumes self_prompter; chatting with the bot's name re-grabs to assistant. Prior ship 2026-04-22: **#30 Bot mode profiles — BT-30b Assistant variants + sticky-return** (`ed49817`) — real `onPlayerOnlineChange` replaces the BT-30a stub. `assistant-server` pauses self_prompter on the first player join (other-count 0→1) and resumes on the last leave (1→0); `assistant-user` pauses/resumes only on the named user (`assistant_user` field in profile JSON, settable atomically via `!botMode assistant-user <username>`); `auto` sticky-return flips runtime survivor→assistant on any join (leave is no-op — BT-30c will own the drop-back via 5-min idle timer). New `reconcileOnSpawn()` handles the boot-time case where players are already online (mineflayer doesn't fire `playerJoined` for pre-existing players). `assistant_user` threaded through `ModeProfile` constructor + getJson/loadJson + atomic dual-writeback (`mode_profile` + `assistant_user`) to profile JSON. `!botMode` extended with optional username param; missing-username on `assistant-user` returns operator chat error with no state change. self_prompter not touched — `pause()` puts state in PAUSED which bypasses the circuit-breaker watchdog by construction (the planned new `stoppedReason` turned out unnecessary). Awaiting live verification: `!botMode assistant-server` pause-on-first-join + resume-on-last-leave; `!botMode assistant-user JP` keys only on JP's events; `!botMode auto` sticky-flips runtime on any join; profile-JSON dual writeback round-trips across restart. Prior ship 2026-04-22: **#30 Bot mode profiles — BT-30a skeleton** (`a903746`) — new `ModeProfile` class at `src/agent/mode_profile.js` (configured/runtime fields, four valid profiles `survivor`/`assistant-server`/`assistant-user`/`auto`, `auto` default if `mode_profile` missing from profile JSON), `!botMode` command added with full Rule-7 perimeter (actions.js + human_delays.INSTANT + chunk_wait.COMMAND_WHITELIST), `bot.on('playerJoined')` and `bot.on('playerLeft')` listeners wired in `agent.js` next to the existing chat/whisper handlers (stub `onPlayerOnlineChange` for now — sticky-return logic lands in BT-30b), profile-path stash `settings.profile_fp` added in `main.js` so `setConfigured` can writeback the updated `mode_profile` field on `!botMode`, invalid-profile path returns operator-facing chat error with no state change. `[ModeProfile]` init + `setConfigured` transition log lines emit on every change. Awaiting next reboot to verify `[ModeProfile] init configured=auto runtime=survivor` log line and `!botMode survivor` writeback. Prior ship 2026-04-21: **#7f Seed-aware Structure Oracle** (cubiomes-WASM + level.dat seed auto-discovery + `!findStructure` command + seed redaction in boot-snapshot) (`d860661`) — modes.js adds skyLight≥8 scope gate to the torch_placing mode update; skills.js `_impl_placeBreadcrumbTorch` fallback adds a solid-reference-block precondition returning `false` (wrapSkill abort) when no geometry exists, preventing the ~5s error loop that spammed placement-stream on every topside session. Prior ship 2026-04-20: **#6 strategic torch placement (left-wall convention)** (`ccf3f53`) — new `placeBreadcrumbTorch` helper in `src/agent/library/skills.js` (~100 lines) plus 1-Hz motion cache on `bot._lastMovement` in `modes.js self_preservation.update()`; strict left-wall torch placement with behind-bot floor fallback for open shafts; `digDown` + `torch_placing` both swapped to the new helper; `goToSurface` warn-logs non-left-wall torches on ascent as a wrong-direction signal. Prior ship 2026-04-20: **#2-follow-up water-breathing potion auto-use** (`eefdb52`) — ~108-line reflex added to `src/agent/modes.js` `self_preservation.update()`, preemptive drown defense at oxygen ≤14 with splash-preferred / drinkable-fallback paths, offhand swap-and-restore, skip-when-shield-raised perimeter. Layers on top of BT-2 L3 turtle-helmet and BT-10g reactive surfacing. Awaiting natural-trigger verification (river/ocean crossing with water_breathing potion in inventory). See Recently completed for the full 2026-04-17 observability bundle.
...
**Status:** 🚧 in-progress — BT-30a (`a903746`) + BT-30b (`ed49817`) + BT-30c (`d8299ce`) + BT-30d (`c61f680`) shipped awaiting live verification; BT-30e (docs close-out) queued. BT-30d landed the Survivor tier-up goal queue: parallel `ModeProfile.survivorQueue` array of `{goal, predicate, source}` entries plus `headIndex` integer. Seed ladder is per-tier (full set per tier before advancing): wood pickaxe/sword/axe → stone pickaxe/sword/axe → iron pickaxe/sword/axe + iron helmet/chestplate/leggings/boots + shield → diamond pickaxe/sword/axe + diamond helmet/chestplate/leggings/boots → netherite pickaxe/sword/axe + netherite helmet/chestplate/leggings/boots (28 entries total, all exported as `SEED_LADDER`). Predicates are plain item-name match `{has: '<name>', min: N}` evaluated against `bot.inventory.slots` (covers main inventory + armor slots 5-8 + offhand slot 45 — items() alone would miss equipped armor). Persistence at `bots/<agent.name>/survivor_queue.json` (mirrors history.js convention) — load on first arm, seed-from-SEED_LADDER if absent or malformed, atomic-ish writeFileSync after every advance/insert. Tick cadence: 30s `SURVIVOR_TICK_MS` `setInterval` (with `.unref()` so it doesn't keep the event loop alive) armed only when `runtime === 'survivor'`; immediate first tick after arming so the head goal pushes without waiting the full interval. Wired arm/disarm into every runtime transition: setConfigured (always disarm, then arm if new runtime is survivor), `_dropToSurvivor` (arm — entering survivor), auto sticky-return (`onPlayerOnlineChange` join branch — disarm), auto chat-return (`noteActivity` survivor→assistant flip — disarm), reconcileOnSpawn (arm for survivor profile and auto-with-no-players, disarm for auto-with-players-online). Each tick checks three signals: (1) defensive disarm if state drifted out of survivor, (2) natural completion of head goal (self_prompter STOPPED + stoppedReason='user' + `_lastPushedGoal` matches head text — LLM called `!endGoal`), (3) predicate-satisfied advance. If neither advance fires and self_prompter is STOPPED with a different last-pushed goal than the current head, push the head via `sp.start(goal)`. Manual operator goal: `!goal "<text>"` while runtime=survivor calls new `manualSurvivorInsert(text)` which splices `{goal, predicate: null, source: 'manual'}` at `headIndex`, persists, resets `_lastPushedGoal`, and force-pushes the new head immediately. Manual entries with predicate=null only advance via natural completion. `actions.js` !goal handler now branches on `agent.mode_profile?.runtime === 'survivor'` — assistant runtimes keep the existing self_prompter.start() path unchanged. New `[ModeProfile]` log lines: `survivorQueue load fp=… queue_len=… head=…` / `survivorQueue seed queue_len=…`, `survivor-timer armed tick_ms=30000 head=… queue_len=…` / `survivor-timer disarmed`, `survivorQueue push reason=… head=… goal=…`, `survivorQueue advance reason=natural_completion|predicate_satisfied head=… goal=…`, `survivorQueue manual-insert head=… goal=… queue_len=…`. Awaiting live verification: cold boot in survivor profile logs `survivor-timer armed` + `survivorQueue seed` + `survivorQueue push reason=initial-push goal="Craft a wooden pickaxe"`; advance fires when bot crafts each ladder item; manual `!goal "build a shelter"` splices at head and pushes immediately. BT-30e underway: docs close-out — three pieces. (1) `!botMode` description in `actions.js` extended to mention the runtime/configured distinction, the survivor goal-queue interaction with `!goal` (manual goals splice at queue head), and the four valid profiles inline (LLM-facing prompt clarity). (2) Profile-JSON schema documented in `profiles/defaults/_default.json` — add `"mode_profile": "auto"` and `"assistant_user": ""` as the canonical schema entries (auto is the default per BT-30a; empty assistant_user is a placeholder for the assistant-user variant). Also explicitly set `"mode_profile": "auto"` in the live `ThatCoolGuyDude.json` so the bot stops relying on the absent-field fallback (today it works because `resolveConfiguredFromProfileJson` returns `auto` with `defaulted: true`, but it logs a warn — explicit is better). (3) Final WB ship entries — once the docs land, move the entire #30 ticket body from `In-progress` → `Shipped — awaiting live verification` (creating that section header since it does not currently exist on the file), flip BT-30e in the sequence to ✅ shipped, bump `Current state` HEAD to the BT-30e code commit. • **Priority:** medium (user-facing control + structured autonomy)
...
**Configured profiles:**
- `survivor` — pure self-sustaining mode. Works through an ordered tier-up goal queue (wood → stone → iron → diamond → netherite for tools, weapons, armor) while reflexes keep the bot alive. The keep-alive layer is largely shipped (self_preservation, BT-10g drowning-escape, BT-10e shield-raise, BT-2-L3 turtle-helmet, #10 survival hardening). New work for this ticket: the queue itself + "what's my current best tier and what's the next goal?" decision logic.
- `assistant-server` — self-prompter paused while ANY player is online. Resumes when the server empties.
- `assistant-user` — self-prompter paused while a named player is online. Resumes when that user logs off.
- `auto` — dynamic mode. Defaults to survivor behavior. When a player logs in, the bot drops what it's doing and switches to assistant runtime (sticky-return). After **30 minutes** of no chat addressed to the bot, runtime drops back to survivor (timer fires regardless of player presence). As soon as the user addresses the bot again, the bot immediately re-enters assistant. The configured profile remains `auto` throughout — only the runtime alternates. "Activity" is chat-to-bot only (mention or DM); ambient in-range chatter between other players does NOT reset the idle timer.
...
**Runtime vs configured.** `survivor`, `assistant-server`, and `assistant-user` have runtime == configured (no internal state machine — they just are what they are). `auto` is the only profile whose runtime alternates between `survivor` and `assistant`, driven by player presence + the **30-min** idle timer. Sticky-return (immediate flip back to assistant on user interaction) lives only inside `auto`.
...
**1. `!setMode` — confirmed, per-flag only.** Registered at `actions.js:386` (INSTANT + COMMAND_WHITELIST already). Wraps `modes.exists / modes.setOn`. Does not address mode groups. **Implication:** new command `!botMode` is a sibling, not a replacement.
...
**`!goal` / `!endGoal` / `!stay` interaction with profiles.** `!goal` (`actions.js:402`) sets `self_prompter.prompt` and starts the loop; `!endGoal` (`:422`) pops-and-advances or stops; `!stay` (`:378`) pauses all modes. **Implication:** Survivor owns the goal queue. Manual `!goal` during Survivor inserts the new goal at the **top** of the queue (operator intent jumps the line; does not flip profile). Lands in BT-30d.

#### Design implications for the next pass

- New class `ModeProfile` with fields `configured` (one of the four profile strings: `survivor` / `assistant-server` / `assistant-user` / `auto`) and `runtime` (current actual behavior — `survivor` or `assistant` only, since the assistant-server / assistant-user variants resolve to assistant at runtime). Sticky-return only fires when `configured === 'auto'`; modifies `runtime` only, never touches `configured`.
- New `!botMode` command added to `actions.js`, `human_delays.INSTANT`, and `chunk_wait.COMMAND_WHITELIST` (Rule 7 perimeter).
...
- Persistence: `ModeProfile.getJson/loadJson` mirrors `ModeController`; serialized next to `self_prompt` state in `history.save()`. Configured-profile changes also writeback to the bot profile JSON's `mode_profile` field on `!botMode` so cold boots see the latest.
...
1. **Survivor tier-up queue schema** — predicate-driven completion (e.g., `sword_tier >= 'stone'` read from `snapshotInventory`), or plain text + LLM judgment? Principle #1 favors predicate-driven. Decide before BT-30d.
...
- **BT-30c.** Auto idle-timeout drop-back: **30-minute** sliding window, runtime drops assistant→survivor (configured stays `auto`). Activity definition is chat addressed to bot only (mention or DM) — ambient in-range chatter does not count. New closure state `lastActivityAt` + `idleTimer` on ModeProfile; `noteActivity(source)` resets the window from agent.js's existing whisper/mention chat handlers; `_startIdleTimer()` arms a 60s `setInterval` only when `configured=auto && runtime=assistant`; on fire, `_dropToSurvivor` flips runtime, logs `[ModeProfile] runtime assistant->survivor reason=auto_idle_timeout idle_ms=...`, and resumes self_prompter (mirror of `_resumeFromAssistant`). Timer fires regardless of player presence (the drop is the whole point of auto). `reconcileOnSpawn` arms the timer when boot lands in auto+assistant. Sticky-return from BT-30b is the inverse path: any chat from the user re-flips runtime and re-pauses self_prompter.
- **BT-30d.** ✅ shipped `c61f680` 2026-04-23. Survivor tier-up goal queue. Per-tier ladder (full tier set before advancing): wood pickaxe/sword/axe → stone pickaxe/sword/axe → iron pickaxe/sword/axe + iron armor full set + shield → diamond pickaxe/sword/axe + diamond armor full set → netherite pickaxe/sword/axe + netherite armor full set. Predicates plain item-name match `{has: '<name>', min: N}` evaluated against `bot.inventory.slots` (covers main inventory + armor slots + offhand — `items()` alone misses equipped armor). Parallel queue on `ModeProfile.survivorQueue = [{goal, predicate, source}, ...]` + `headIndex` integer; only current head pushed to self_prompter via `start(goal)`. Persistence: `bots/<agent.name>/survivor_queue.json` (mirrors history.js path convention) — load lazily on first arm, seed-from-`SEED_LADDER` if missing/malformed, atomic-ish writeFileSync after every advance/insert. Tick: 30s `SURVIVOR_TICK_MS` `setInterval` (with `.unref()`) armed only when `runtime === 'survivor'`; disarmed in setConfigured (always-then-arm-if-survivor) + auto sticky-return + auto chat-return; armed in `_dropToSurvivor` + reconcileOnSpawn (survivor branch + auto-with-no-players branch). Each tick: defensive disarm if state drifted out of survivor; natural-completion advance (self_prompter STOPPED + stoppedReason='user' + `_lastPushedGoal` matches head); predicate-satisfied advance; otherwise initial-push if self_prompter is STOPPED. Manual `!goal "<text>"` while runtime=survivor calls `manualSurvivorInsert(text)` which splices `{goal, predicate: null, source: 'manual'}` at `headIndex`, persists, force-pushes immediately. Null predicate means advance only via natural completion. `actions.js` !goal handler branches on `runtime === 'survivor'`; assistant runtimes unchanged.
...
**Fix (two guards, complementary)**

*Guard 1 — scope the mode to likely tunnel/underground via sky-light (`src/agent/modes.js` `torch_placing` update block):*
- Read `bot.world.getSkyLight(bot.entity.position.floored())` before calling `placeBreadcrumbTorch`.
- If `skyLight >= 8`, skip this tick (bot is effectively outside / well-lit by sky — breadcrumb torches are the wrong mode here).
- Rationale: `skyLight` is day/night-independent (Minecraft computes it as sky-propagation at build time), so it cleanly discriminates "roof over head / deep enough for breadcrumbs" from "topside or well-lit area". Threshold `8` matches vanilla mob-spawn darkness boundary.

*Guard 2 — geometry precondition in the fallback path (`src/agent/library/skills.js` `_impl_placeBreadcrumbTorch` ~5292):*
- Before attempting the behind-bot fallback placement, probe the block directly below `(torchX, torchY-1, torchZ)` for solidity.
...
2. Code ship: modes.js sky-light guard + skills.js geometry precondition + skill-stream reason tagging
3. WB: move body `In-progress` → `Shipped — awaiting live verification`; bump `Current state` HEAD + ship note
...
**Status:** ✅ verified 2026-04-21 — POI memory + jsonl populated (6 entries in `bots/ThatCoolGuyDude/poi_memory.json` across `scanner` / `village_scanner` / `zone` sources, matching `data/poi-stream.jsonl` register events). Watcher source untested (awaits a newly-lit portal event; non-blocking).
