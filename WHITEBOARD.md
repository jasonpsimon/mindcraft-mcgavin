...
_Last updated: 2026-04-17 (BT-bundle(b) Process exit reasons shipped `46024a6` and live-deployed on top of BT-bundle(a). Two files, 20 lines of real change: (1) `src/process/init_agent.js` — three module-top-level handlers above the IIFE: `process.on('exit', code => ...)` → `[Exit] event=process_exit code=<n>` (synchronous, fires for every exit path Node naturally reaches); `process.on('uncaughtException', err => ...)` → `[Exit] event=uncaught err="..." stack_head="..."` then `process.exit(1)`; `process.on('unhandledRejection', reason => ...)` → `[Exit] event=unhandled_rejection reason="..."` then `process.exit(1)`. (2) `src/agent/agent.js` `cleanKill(msg, code)` — one prepended `console.log('[Exit] event=clean_kill code=<n> reason="<msg>"')` before the history/chat/save cascade. **Restart-semantics gotcha caught before deploy:** registering a listener on `uncaughtException` / `unhandledRejection` suppresses Node's default crash-exit code (1); `agent_process.js:31` parent-side restart logic is `if (code !== 0 && signal !== 'SIGINT') { restart }`, so a code=0 exit would silently break crash-restart. Fix: explicit `process.exit(1)` at the end of both handlers — synthetic re-verified `exit_code=1`. **Node signal gotcha documented as out-of-scope:** SIGTERM/SIGINT don't reach `process.on('exit')` in Node when no signal handler is registered (tested via `/tmp/test_signals.mjs`); accepted per the "no shutdown-semantics changes" scope boundary. Synthetic-verified three scenarios (clean-exit, uncaught throw, unhandled rejection) via scratch-file tests; live: bot restarted on `46024a6` (PID 2849076), handlers installed, awaiting natural cleanKill trigger per the established BT-6/BT-9/BT-10/BT-bundle(a) acceptance pattern. Next: BT-bundle(c) File I/O silent-swallow audit — the final BT-bundle item.)_
...
_(empty — BT-bundle(b) Process exit reasons shipped `46024a6` and live-deployed 2026-04-17 on top of BT-bundle(a). See Recently completed. Next in the logging roadmap: BT-bundle(c) File I/O silent-swallow audit — the final BT-bundle item.)_
