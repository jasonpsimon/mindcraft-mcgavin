...
_Last updated: 2026-04-17 (BT-3b LLM adapter sweep **shipped** `8580a11`. Closes the Principle 5 migration started by BT-3 — every LLM round-trip across the fleet now emits one uniform `[LLM]` structured line per call. 18 adapter files touched (`cerebras, claude, deepseek, gemini, glhf, gpt, grok, groq, huggingface, hyperbolic, mercury, mistral, novita, ollama, openrouter, qwen, replicate, vllm`) + `azure.js` inherits via `extends GPT` (zero direct edit — `gpt.js` uses `this.constructor.name` as the label so AzureGPT surfaces as `label=AzureGPT` automatically). 344 insertions, 108 deletions. 12 Shape A adapters use the default OpenAI-compat `extractUsage`; 7 Shape B adapters ship custom extractors (`_extractClaudeUsage` / `_extractGeminiUsage` / `_extractOllamaUsage` / `_extractHyperbolicUsage` / default for mistral / null-token streaming for replicate + huggingface). Internal-retry-loop adapters (`glhf`, `qwen-embed`, `ollama`, `hyperbolic`, `huggingface`) place the wrap inside the loop so each attempt emits one `[LLM]` line per round-trip — same per-attempt convention BT-3 set on `lmstudio.js`. Verified: `node --check` clean on all 19; dynamic import resolves on all 19; synthetic log-capture passed for one Shape A (Mercury, OpenAI shape) + one Shape B (Claude, Anthropic shape); live regression on gaming server captured 115 `[LLM]` lines from `LMStudio-Embed` post-restart with format byte-identical to pre-BT-3b. Observability side of the roadmap now has **zero** known gaps on active paths — BT-7b (~72 remaining skill wraps) is the only migration follow-up still open, and is intentionally trigger-gated on live-data review.)_
...
- **withLLMMetrics** (BT-3 + BT-3b): one `[LLM]` structured log line per LLM round-trip (chat + embed) across **every** adapter in `src/models/*.js`. Fields: `label, model, elapsed_ms, prompt_tok, completion_tok, total_tok, tok_per_s, retries, finish, cache_hit, status`. Terminal errors emit `status=error err_class=...`. BT-3 migrated `lmstudio.js`; BT-3b `8580a11` landed the remaining sweep — 18 files touched + `azure.js` inherits from GPT via `this.constructor.name`-driven labeling, so AzureGPT surfaces as `label=AzureGPT` with zero direct edit. 12 Shape A adapters use the default OpenAI-compat `extractUsage`; 7 Shape B adapters (claude, gemini, mistral, replicate, ollama, hyperbolic, huggingface) ship custom extractors or null-token streaming. Internal-retry-loop adapters (`glhf`, `qwen`-embed, `ollama`, `hyperbolic`, `huggingface`) place the wrap inside the think-block loop so each attempt emits exactly one `[LLM]` line per round-trip. Principle 5 migration **complete** at the model-fleet layer.
