...
_Last updated: 2026-04-17 (BT-7b Skill lifecycle sweep **shipped** `e563456`. Closes the Principle 5 migration started by BT-7 — every public skill export in `src/agent/library/skills.js` now emits one uniform `[Skill] name=… args=… ms=… outcome=success|abort|error [err_class=…]` lifecycle line per call. 31 new `wrapSkill` wraps across 3 waves (wave 1 combat/safety: `escapeSpawnZone, escapeProtectedZone, attackNearest, attackEntity, defendSelf, avoidEnemies`; wave 2 movement: `goToPosition, goToNearestEntity, goToPlayer, followPlayer, moveAway, moveAwayFromEntity, stay, useDoor, goToBed, goToSurface`; wave 3 world/chest/trade: `clearNearestFurnace, breakBlockAt, autoBreakStuckPlant, discard, putInChest, takeFromChest, viewChest, consume, giveToPlayer, tillAndSow, activateNearestBlock, showVillagerTrades, tradeWithVillager, placeTorchAt, useToolOn`). 62 insertions, 31 deletions. Skip list (utility primitives, kept raw): `wait` and `goToGoal`. Final inventory: 42 wrapped (11 pre-existing BT-7 + 31 new BT-7b), all 51 public async skill exports covered at the lifecycle layer modulo the documented skip list. Verified: `node --check` clean on the modified file; dynamic import resolves 51 exports with 0 missing; synthetic three-branch lifecycle test captured canonical `[Skill]` format on success/abort/error (with `err_class=TypeError` rider on throws); live regression on gaming server post-restart appended 49 new records to `data/skill-stream.jsonl` — `autoBreakStuckPlant` (×34, all abort) and `defendSelf` (×15: 13 abort, 2 success) fired in-engine with format byte-identical to pre-BT-7b BT-7 records. Observability migration phase is now **fully closed** — BT-1 through BT-12, BT-bundle(a/b/c), BT-3b, and BT-7b all shipped; nothing trigger-gated remains.)_
