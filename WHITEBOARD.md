...
_Last updated: 2026-04-17 (#26 phantom self_defense fixed **shipped** `df9b737`. One-line bug fix in `src/agent/modes.js:171` — `self_defense` mode's hostile-detection range narrowed `16` → `8` to match `defendSelf`'s attack range. Mode was triggering on hostiles in the 8-16 ring that the skill's attack range couldn't reach, producing `say(agent, 'Fighting X!')` + `execute()` followed by an immediate `outcome=abort` in `defendSelf` with `notes="No enemies nearby to defend self from"`. Found within minutes of BT-7b telemetry going live — `data/skill-stream.jsonl` showed 241 `defendSelf` aborts in a tight window paired with 11 `[Boot]` snapshots; the misalignment was greppable as soon as BT-7b made every skill emit `outcome=abort`. **Triple-win on verification:** the one-line change resolved three previously-reported symptoms simultaneously over a 3-minute observation window — phantom combat went from ~1 chat announcement/sec to 0, `disconnect.spam` reconnect storm went from 10 kicks to 0, and the stuck `escapeProtectedZone` mutex holder unwedged (mutex.holder=null, pathfinder.active=false, position escaped from stuck (-137,72,164) to idle (-169,69,192)). The reconnect storm and the stuck escape were both symptoms of the phantom mode trigger blowing up `interrupts: ['all']` on every tick. Direct payoff for BT-7b: a class of phantom-action bugs (mode triggers skill, skill aborts immediately) is now mechanically findable across all 42 wrapped skills via `outcome=abort` clustering. Previous BT-7b ship `e563456` closed the Principle 5 migration BT-7 started — every public skill export in `src/agent/library/skills.js` now emits one uniform `[Skill] name=… args=… ms=… outcome=success|abort|error [err_class=…]` lifecycle line per call. Observability migration phase fully closed at the library level — BT-1 through BT-12, BT-bundle(a/b/c), BT-3b, and BT-7b all shipped; #26 is the first migration-discovered bug fix.)_
...
- Branch: `develop` — HEAD `df9b737`. Eighteen observability items shipped on `develop` today plus one migration-discovered bug fix (#26 phantom self_defense): BT-1 StateTicker, BT-8 BootSnapshot, BT-3 LLM call telemetry + BT-3b LLM adapter sweep across the remaining 18 files (azure inherits) — every adapter in `src/models/*.js` emits uniform `[LLM]` round-trip lines. BT-2 DamageStream, BT-12 startup-window ordering fix, BT-4 MemoryRecall, BT-5 AutoRecovery stats, BT-7 Skill lifecycle + **BT-7b Skill lifecycle sweep across the remaining 31 exports in `src/agent/library/skills.js`** — every public async skill export in the agent library (42 of 51, with the documented two-primitive skip list) now emits one uniform `[Skill]` lifecycle line per call, closing the BT-7 migration at the library level. Goal lifecycle, BT-6 Pathfinder telemetry, BT-11 ContextBuilder truncation decisions, BT-9 World/time events, BT-10 Entity delta stream, BT-bundle(a) Mutex wait duration, BT-bundle(b) Process exit reasons, and BT-bundle(c) File I/O silent-swallow. Pushed to `origin/develop` 2026-04-17. **The lifecycle layer (BT-7 + BT-7b skills + Goal + BT-6 paths) sits underneath the measurement layer (BT-5) as the two-tier observability story, with BT-11 closing the prompt-construction counterpart alongside BT-4. BT-9 + BT-10 together surface ambient world/entity state (weather / time / dimension / nearby hostile lifecycle) that was previously invisible to log tailers. BT-bundle(a) adds a latency number to the existing `[BotMutex]` queue-depth log so concurrency-bug signal (growing queue) now pairs with a scale signal (how long acquires actually waited). BT-bundle(b) closes the end-of-session observability gap — every shutdown path that Node naturally reaches (`cleanKill`, uncaught throw, unhandled rejection, natural event-loop drain) now emits a structured `[Exit]` line with reason paired to exit code, so session-replay readers see *why* and *how* the tmux buffer ended instead of a dropped cursor. BT-bundle(c) closes the last unaudited silent-failure surface — six file-I/O catch branches tightened (one of them a real latent bug in `prompter.js:134` where a callback was being silently discarded on a sync `writeFileSync`). Every user-visible file-I/O operation now emits a structured `[<Subsystem>] ...` line on failure, matching the `[StateTicker]` / `[BootSnapshot]` / `[LLM]` / `[Exit]` conventions. BT-3b lands the LLM-fleet migration piece: every LLM adapter now ships uniform `[LLM]` telemetry regardless of provider. BT-7b lands the skill-library migration piece: every public skill export now ships uniform `[Skill]` telemetry regardless of category. The observability migration phase (BT-bundle + BT-3/BT-3b + BT-7/BT-7b) is now fully closed — nothing trigger-gated remains.**
