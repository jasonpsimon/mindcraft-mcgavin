...
**Status:** 🚧 in-progress — BT-30a (`a903746`) + BT-30b (`ed49817`) + BT-30c (`d8299ce`) shipped awaiting live verification; BT-30d underway (Survivor tier-up goal queue). Per-tier ladder (full set of one tier completes before advancing): wood → stone → iron (+armor+shield) → diamond (+armor) → netherite (+armor). Predicate format: plain item-name match `{has: '<item_name>', min: N}` evaluated against `bot.inventory.items()` — no tier abstraction, no DSL. Parallel queue lives on `ModeProfile.survivorQueue` (array of `{goal, predicate}` objects + `head_index`); only the current head goal is pushed to self_prompter via `setPrompt + start`. State persists to a separate file `bots/<profile_name>/survivor_queue.json` (NOT in profile JSON, NOT in memory only) — survives restart, reloaded on boot; if file is missing or malformed, write the seed ladder. Tick cadence: 30s `setInterval` armed only when `runtime === 'survivor'` and `configured` is `survivor` or `auto`. On each tick: re-evaluate current head's predicate; if satisfied, advance `head_index`, persist file, push next goal; if self_prompter is STOPPED with no prompt, push current head. Manual `!goal "<text>"` while runtime=survivor inserts `{goal: text, predicate: null}` at queue[0], persists, and immediately pushes — predicate=null means advance only when self_prompter naturally completes (LLM-signaled). When runtime=assistant, `!goal` falls through to the existing raw self_prompter path. New `[ModeProfile]` log lines: `survivorQueue load/seed`, `survivorQueue advance head_index=X→Y goal=...`, `survivorQueue push goal=...`, `survivorQueue manual-insert source=...`. Seed ladder is a starting point — JP can hand-edit `survivor_queue.json` between sessions to add leather armor, bow, or reorder. BT-30e (docs close-out) remains queued. • **Priority:** medium (user-facing control + structured autonomy)
...
- **BT-30d.** Survivor tier-up goal queue. Per-tier ladder (full tier set before advancing): wood pickaxe/sword/axe → stone pickaxe/sword/axe → iron pickaxe/sword/axe + iron armor full set + shield → diamond pickaxe/sword/axe + diamond armor full set → netherite pickaxe/sword/axe + netherite armor full set. Predicates plain item-name match: `{has: '<name>', min: N}` evaluated against `bot.inventory.items()`. Parallel queue on `ModeProfile.survivorQueue = [{goal, predicate}, ...]` + `head_index` integer; only current head pushed to self_prompter via `setPrompt + start`. Persistence: `bots/<profile_name>/survivor_queue.json` — load on boot, write seed ladder if missing/malformed, atomic writeback after every advance/insert. Tick: 30s `setInterval` armed only when `runtime === 'survivor'` && `configured` ∈ {survivor, auto}; disarmed in setConfigured + drop/return paths. Each tick re-evaluates head predicate, advances on satisfaction, pushes if self_prompter is STOPPED with no prompt. Manual `!goal` while runtime=survivor inserts `{goal: text, predicate: null}` at queue[0] (operator jumps the line, doesn't override profile); null predicate means advance only when self_prompter naturally completes (LLM-signaled). Manual `!goal` while runtime=assistant falls through to existing raw self_prompter behavior unchanged.
