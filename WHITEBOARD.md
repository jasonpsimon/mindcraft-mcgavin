...
**Status:** 🚧 in-progress — BT-30a (`a903746`) + BT-30b (`ed49817`) shipped awaiting live verification; BT-30c underway (auto idle-timeout drop-back). Activity definition: chat addressed to bot only (mention or DM) — in-range chatter between other players does NOT count. Sliding **30-minute** window; `lastActivityAt` resets on every counted activity. Idle timer is armed only when `configured=auto && runtime=assistant`; check loop runs at 60s cadence and fires regardless of player presence (the drop is the whole point of auto). On fire, runtime flips assistant→survivor and self_prompter is resumed (mirrors `_resumeFromAssistant`). `reconcileOnSpawn` arms the timer when boot lands in auto+assistant. New log line `[ModeProfile] runtime assistant->survivor reason=auto_idle_timeout idle_ms=...` on every drop. BT-30d (Survivor goal queue) and BT-30e (docs close-out) remain queued. • **Priority:** medium (user-facing control + structured autonomy)
