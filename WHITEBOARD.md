...
_(none — BT-30 between sub-tickets; BT-30d queued.)_

## Shipped — awaiting live verification

### 30. Bot mode profiles: survivor / assistant-server / assistant-user / auto

**Status:** 🚢 shipped 2026-04-22 awaiting live verification (`d8299ce`, BT-30c — auto idle-timeout drop-back; layered on BT-30b `ed49817` + BT-30a `a903746`) — 30-minute sliding window of chat-to-bot activity. New `noteActivity(source)` on ModeProfile records every qualifying event and resets `lastActivityAt`; new `_startIdleTimer/_stopIdleTimer/_checkIdle` handle the 60s-cadence interval; new `_dropToSurvivor(reason)` flips runtime assistant→survivor + resumes self_prompter (mirror of `_resumeFromAssistant`'s pause-skip semantics). Timer is armed only when `configured=auto && runtime=assistant` and disarmed on every other transition (setConfigured, drop, reconcile out of assistant). Drop fires regardless of player presence — `auto`'s whole point is letting players stay connected without holding the bot. `noteActivity` also handles auto chat-return: if a chat-to-bot lands while auto is in survivor (idle drop already happened), runtime flips back to assistant and pauses self_prompter immediately — the inverse of the idle drop. agent.js: whispers always count as activity (DM by definition); open chat counts only when the message contains the bot's name (case-insensitive substring) — ambient in-range chatter does NOT reset the window. `reconcileOnSpawn` auto branch arms the timer when boot lands in auto+assistant. `[ModeProfile] runtime assistant->survivor reason=auto_idle_timeout idle_ms=...` log on every drop; `auto_chat_return` log on every re-grab. BT-30d (Survivor goal queue) and BT-30e (docs close-out) remain queued. • **Priority:** medium (user-facing control + structured autonomy)
